    ...
    <div className="p-5 pb-24">
      <div className="mb-5 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-black active:scale-[0.97] transition-transform"
        >
          <Plus size={16} /> Create
        </button>
      </div>
      ...
      <div className="space-y-3">
        {allPlans.map((plan) => (
          <div key={plan.id} className="overflow-hidden rounded-2xl border border-border bg-surface">
            <button
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="flex items-center gap-1 rounded-lg bg-primary/15 px-3 py-1 text-xs font-bold text-primary"
                        >
                          <Play size={12} /> Start
                        </button>
                        {expandedDay === day.id ? <ChevronUp size={14} className="text-muted" /> : <ChevronDown size={14} className="text-muted" />}
                      </div>
                    </button>
                    {expandedDay === day.id && (
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-bg">
      <div className="flex items-center justify-between border-b border-border p-5">
        ...
        <div className="flex gap-2">
          <button onClick={onClose} className="rounded-xl border border-border bg-surface px-4 py-2 text-sm font-medium text-muted">
            Cancel
          </button>
          <button onClick={save} className="rounded-xl bg-primary px-4 py-2 text-sm font-bold text-black">
            Save
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        <div>
          <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-widest text-muted">Plan Name</label>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="e.g. My Push Pull Legs"
            className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm text-text outline-none placeholder:text-muted/50 focus:border-primary"
          />
        </div>
        <div>
          <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-widest text-muted">Description</label>
          <input
            value={description}
        ...
        {days.map((day, dayIdx) => (
          <div key={day.id} className="rounded-2xl border border-border bg-surface p-4">
            <input
              ...
              className="mb-3 bg-transparent font-bold text-text outline-none"
            />
            {day.exercises.map((pe, exIdx) => (
              <div key={exIdx} className="mb-2 flex items-center justify-between rounded-lg bg-surface-2 px-3 py-2.5 text-sm">
                <span className="text-muted">{pe.exerciseName} ({pe.sets} x {pe.reps})</span>
            ...
            <button
              ...
              className="mt-2 w-full rounded-xl border border-dashed border-primary/30 py-2.5 text-xs font-semibold text-primary"
            >
              + Add Exercise
            </button>
          </div>
    ...
    <div className="flex h-full flex-col bg-bg">
      <div className="flex items-center gap-2 border-b border-border p-4">
        <input
          ...
          className="flex-1 rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none placeholder:text-muted/50 focus:border-primary"
          autoFocus
        />
        <button onClick={onClose} className="text-sm font-medium text-muted">Cancel</button>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-1">
        {filtered.map((ex) => (
          <button
            ...
            className="w-full rounded-xl border border-border bg-surface p-3 text-left text-sm transition-colors hover:border-primary"
          >
